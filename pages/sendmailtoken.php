...
# This page is called to send a change mail token by old mail
...
if ( $result === "" ) {
    ...
    $reset_url .= "?action=resetmailbytoken&token=$token";
    ...
    # Send message
    if ( send_mail($mailer, $mail, $mail_from, $mail_from_name, $messages["resetmailsubject"], $messages["resetmailmessage"].$mail_signature, $data) ) {
        $result = "tokensent";
    } else {
        $result = "tokennotsent";
        error_log("Error while sending mail token to $mail (user $login)");
    }
}
