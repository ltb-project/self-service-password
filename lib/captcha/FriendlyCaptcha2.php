...
class FriendlyCaptcha2
{
    ...
    function generate_js_captcha(){
        $captcha_js = '
            <script
                type="module"
                src="https://cdn.jsdelivr.net/npm/@friendlycaptcha/sdk@0.2.0/site.min.js"
                async
                defer
            ></script>
    ...
    function generate_html_captcha($messages, $lang){
        $endpoint = "";
        if (strpos($this->friendlycaptcha_apiurl, "eu.frcapi.com") !== false) {
            $endpoint = ' data-api-endpoint="eu"';
        }
        ...
        <div class="row mb-3">
            ...
            <div class="col-sm-8">
            <div class="frc-captcha" data-sitekey="'.$this->friendlycaptcha_sitekey.'"'.$endpoint.'></div>
            </div>
        </div>';
    ...
    function verify_captcha_challenge(){
        $result="";
        if (isset($_POST["frc-captcha-response"]) and $_POST["frc-captcha-response"]) {
            $captchaphrase = strval($_POST["frc-captcha-response"]);
            ...
            $options = [
                'http' => [
                    ...
                    "X-API-Key: " . $this->friendlycaptcha_secret . "\r\n",
                    'method' => 'POST',
