...
class Application
{
    ...
    private function isActionAllowed($action)
    {
        // Available actions
        $availableActions = [];
        if ($this->config['use_change']) {
            array_push($availableActions, "change");
        }
        if ($this->config['change_sshkey']) {
            array_push($availableActions, "changesshkey");
        }
        if ($this->config['use_questions']) {
            array_push($availableActions, "resetbyquestions", "setquestions");
        }
        if ($this->config['use_tokens']) {
            array_push($availableActions, "resetbytoken", "sendtoken");
        }
        if ($this->config['use_sms']) {
            array_push($availableActions, "resetbytoken", "sendsms");
        }
    ...
    public function loadLanguages($lang, $allowedLangs, &$oldMessages)
    {
        ...
        // Available languages
        $languages = $languageSelector->findAvailableLanguages(__DIR__.'/../lang', $lang, $allowedLangs);
        ...
        return array_merge($oldMessages, $messages);
    }

    /**
     * @param string $containerPath
     * @param string $containerOverridePath
     *
     * @return Container
     */
